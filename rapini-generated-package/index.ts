...
function makeRequests(axios: AxiosInstance, config?: AxiosConfig) {
    return {
        ...
        createPets: (payload: Pet) => axios.request<unknown>({
            method: "post",
            url: `/pets`,
            data: payload
        }).then(res => res.data),
        showPetById: (petId: string) => axios.request<Pets>({
            method: "get",
            url: `/pets/${petId}`
        }).then(res => res.data),
        listPetPhotos: (petId: string) => axios.request<Photos>({
            method: "get",
            url: `/pets/${petId}/photos`
        }).then(res => res.data),
        postPetPhoto: (payload: Photo, petId: string) => axios.request<unknown>({
            method: "post",
            url: `/pets/${petId}/photos`,
            data: payload
        }).then(res => res.data)
...
function makeQueries(requests: ReturnType<typeof makeRequests>, queryIds: ReturnType<typeof makeQueryIds>) {
    return {
        ...
        useListPetPhotos: (petId: string, options?: Omit<UseQueryOptions<Awaited<ReturnType<typeof requests.listPetPhotos>>, unknown, Awaited<ReturnType<typeof requests.listPetPhotos>>, ReturnType<(typeof queryIds)["listPetPhotos"]>>, "queryKey" | "queryFn">): UseQueryResult<Awaited<ReturnType<typeof requests.listPetPhotos>>, unknown> => useQuery(queryIds.listPetPhotos(petId), () => requests.listPetPhotos(petId), options)
    } as const;
}
type MutationConfigs = {
    useCreatePets?: (queryClient: QueryClient) => Pick<UseMutationOptions<Awaited<ReturnType<ReturnType<typeof makeRequests>["createPets"]>>, unknown, Parameters<ReturnType<typeof makeRequests>["createPets"]>[0], unknown>, "onSuccess" | "onSettled" | "onError">;
    usePostPetPhoto?: (queryClient: QueryClient) => Pick<UseMutationOptions<Awaited<ReturnType<ReturnType<typeof makeRequests>["postPetPhoto"]>>, unknown, Parameters<ReturnType<typeof makeRequests>["postPetPhoto"]>[0], unknown>, "onSuccess" | "onSettled" | "onError">;
};
function makeMutations(requests: ReturnType<typeof makeRequests>, config?: Config["mutations"]) {
    return {
        useCreatePets: (options?: Omit<UseMutationOptions<Awaited<ReturnType<typeof requests.createPets>>, unknown, Parameters<typeof requests.createPets>[0], unknown>, "mutationFn">) => useRapiniMutation<Awaited<ReturnType<typeof requests.createPets>>, unknown, Parameters<typeof requests.createPets>[0]>(payload => requests.createPets(payload), config?.useCreatePets, options),
        usePostPetPhoto: (petId: string, options?: Omit<UseMutationOptions<Awaited<ReturnType<typeof requests.postPetPhoto>>, unknown, Parameters<typeof requests.postPetPhoto>[0], unknown>, "mutationFn">) => useRapiniMutation<Awaited<ReturnType<typeof requests.postPetPhoto>>, unknown, Parameters<typeof requests.postPetPhoto>[0]>(payload => requests.postPetPhoto(payload, petId), config?.usePostPetPhoto, options)
    } as const;
}
